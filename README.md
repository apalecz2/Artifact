...
4. **Provenance**: a deterministic grid-based matcher links each output cell back to its source OCR word(s) in the document, using column/row geometry with fallback passes for irregular layouts.
...
All document processing runs on-device: no document data ever leaves the machine. The only external network activity is the one-time first-run setup wizard, which downloads the OCR engine, PDF renderer, llama-server binary, and GGUF model files (~3.5 GB total).
...
website/      # Marketing/landing site, independent of the app
...
Large dependencies (Tesseract, PDFium, llama-server, GGUF models) are not bundled in the installer. On first launch an in-app wizard detects your hardware and downloads everything automatically to the OS app-data directory.
...
[Elastic License 2.0](LICENSE). Third-party components are listed in [NOTICES.md](NOTICES.md).
