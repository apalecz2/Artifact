...
/**
 * Minimal Markdown renderer for the legal pages — mirrors the app's renderer but
 * uses ordinary anchors (no Tauri). Supports only the constructs the Privacy and
 * EULA documents use (headings, paragraphs, lists, bold/italic/code, links, rules),
 * so it needs no third-party Markdown dependency. Documents with tables (NOTICES)
 * are rendered as preformatted text by LegalPage instead.
 */

// Relative cross-references between the canonical docs → site routes.
...
function renderInline(text: string, keyPrefix: string): React.ReactNode[] {
    ...
    while ((match = pattern.exec(text)) !== null) {
        ...
        if (match[1] !== undefined) {
            ...
            nodes.push(<code key={key} className="rounded bg-surface-container-high px-1 py-0.5 font-mono text-[0.85em]">{match[3]}</code>);
...
export default function Markdown({ source }: { source: string }): React.ReactElement {
    ...
    const flushList = () => {
        ...
        blocks.push(
            <ul key={`ul-${key++}`} className="my-3 flex list-disc flex-col gap-1.5 pl-6 text-on-surface-variant">
                {items.map((item, idx) => (
                    <li key={idx} className="font-body-md text-body-md leading-relaxed">{renderInline(item, `li-${key}-${idx}`)}</li>
                ))}
            </ul>,
        );
    };
    ...
    for (const raw of lines) {
        ...
        if (heading) {
            ...
            const cls = level === 1
                ? 'font-display-sm text-headline-lg sm:text-display-sm text-primary tracking-tight mt-2 mb-4'
                : level === 2
